...
colsHC <- colorRampPalette(pal_HC)
colsHCd <- colorRampPalette(pal_HCd)
...
blues <- colorRampPalette(c(pal_my[c(1, 15)])) ## "#d9e1fe" = light grayish blue ##
cyans <- colorRampPalette(c(pal_my[1], pal_my[2], pal_my[11]))
grblues <- colorRampPalette(c(pal_my[17], pal_my[16]))
grays <- colorRampPalette(pal_my[c(2, 18, 19)])
grays2 <- colorRampPalette(pal_my[c(1, 18, 20)])
grblues2 <- colorRampPalette(c("#d9e1fe", pal_my[16]), alpha=T)
blues2 <- colorRampPalette(c("#ffffff", pal_my[14])) ## "#d9e1fe" = light grayish blue ##
whcybl <- colorRampPalette(pal_my[c(1, 12, 16)])
whblmg <- colorRampPalette(pal_my[c(1, 16, 4)])
blgrmg <- colorRampPalette(pal_my[c(16, 2, 5)])
cols <- colorRampPalette(pal_my)
cols1 <- colorRampPalette(pal_my[3:19])
cols2 <- colorRampPalette(mypal2)
cols3 <- colorRampPalette(pal_my[3:16])
cols4 <- colorRampPalette(pal_my[c(3:5, 10, 12:16)])
pdxcols <- colorRampPalette(pdxpal[c(3, 1)])


pal_sci <- c("#771111", "#f7ee55", "#117755", "#44aa88", "#114477", "#1965b0", "#771144", "#aa4477")
sci <- colorRampPalette(pal_sci)

#'
#' ## **`mpal()`**
#'
mpal <- function(x, p = sci, a = 1, ...) { ## Random color palette generation, using palette specified from '`colorRampPalette(n)`', with '`n`' determined by length of, or number of rows in, '`x`' ##
...
R.mpathDia <- function(ivars, mvars, dvars, direct = c("east", "south", "south", "west"), xlim = c(-0.5, 1.25), ylim = c(-0.5, 1.25), bg = NA, arrcol = pal_my[20], ptitle = NULL, lwd = 1, lty.c = 2, ...) { ## ivars and dvars = lists of lengths 1:n containing objects of class "manifest" or "latent", respectively... dir is used for the arrows, the remaining args set various graphical parameter options ##
    ...
    for (d in 1:ndep) {
        ...
        for (m in 1:nmed) {
            pathdiagram::arrow(from = mvars[[m]], to = dvars[[d]], start = direct[3], end = direct[4], col = arrcol, lwd = lwd, lty = 1)
            ## (dashed) arrows from predictor to dep. vars ##
            for (i in 1:nind)
                pathdiagram::arrow(from = ivars[[i]], to = dvars[[d]], start = direct[1], end = direct[4], col = arrcol, lwd = lwd, lty = lty.c)
            pathdiagram::arrow(from = ivars[[i]], to = mvars[[m]], start = direct[1], end = direct[2], col = arrcol, lwd = lwd, lty = 1)}}
    ## add title if user specified a title ##
    if (!is.null(ptitle)) {title(ptitle)}
}
#'
#' ## **`R.search()`**
#'
