...
library(car) ## recode() ##
dat <- read.spss("../CLASS-CDA/data/reuters.sav", to.data.frame = TRUE)
...
```{r echo=FALSE}
kable(Rmsmm(dat$party), caption = "Summary information for '**party**'
...
dat <- na.omit(dat)
sapply(dat, Risna) ## bye-bye NAs! ... again ##
...
Here we are taking one of the favorability proportions (i.e. "success proportions") and comparing it to the _Null Hypothesis_ ($H_{0}$) represented by $\pi_{0}$ (i.e., 0.50) [@agresti2013categorical; @agresti1998approx].
